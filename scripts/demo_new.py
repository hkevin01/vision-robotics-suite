...
from vision_systems.camera_calibration import CameraCalibrator  # noqa: E402
from vision_systems.halcon_algorithms import HalconProcessor  # noqa: E402
...
def demo_vision_processing() -> bool:
...
def demo_camera_calibration() -> bool:
    ...
    for i in range(10):  # Simulate 10 calibration images
        # In real application, use actual detected corners
...
def demo_quality_analysis() -> bool:
    ...
    measurements = [
        {"parameter": "diameter", "value": 25.05, "target": 25.0,
         "tolerance": 0.1},
        {"parameter": "length", "value": 49.98, "target": 50.0,
         "tolerance": 0.2},
        {"parameter": "width", "value": 24.97, "target": 25.0,
         "tolerance": 0.15},
        {"parameter": "surface_roughness", "value": 1.2, "target": 1.0,
         "tolerance": 0.5},
    ]
    ...
    for measurement in measurements:
        ...
        target = float(measurement["target"])
        ...
        logger.info(f"  {param}: {value:.2f} (target: {target:.2f} "
                    f"±{tolerance:.2f}) {status}")
...
def demo_system_integration() -> bool:
...
def main() -> None:
    ...
    logger.info(f"\n📊 Results: {successful_demos}/{total_demos} "
                f"demos completed successfully")

    if successful_demos == total_demos:
        logger.info("🎉 All demonstrations completed successfully!")
        logger.info("🔧 System is ready for industrial deployment")
    else:
        logger.warning("⚠️  Some demonstrations failed - "
